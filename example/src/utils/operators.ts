...
export const operators = {
  ...
  '*': ((args: Program): ProgramOutput => {
    ...
    for (const item of list.slice(1)) {
      cost +=
        costs.mulPerOp +
        (BigInt(item.atom.length) + BigInt(size)) * costs.mulLinearPerByte +
  ...
  'pubkey_for_exp': ((args: Program): ProgramOutput => {
    ...
    const cost =
      costs.pubkeyBase + BigInt(list[0].atom.length) * costs.pubkeyPerByte;
    return mallocCost({
  ...
  'lognot': ((args: Program): ProgramOutput => {
    ...
    const cost =
      costs.lognotBase + BigInt(items[0].atom.length) * costs.lognotPerByte;
    return mallocCost({
...
export function toList(
  ...
  if (typeof length === 'number' && list.length !== length)
    ...
    throw new Error(
      ...
      } arguments in ${JSON.stringify(name)} operator${program.positionSuffix}.`
    );
  if (type !== undefined)
    list.forEach((item) => {
      if (
        ...
        throw new Error(
          `Expected ${type} argument in ${JSON.stringify(name)} operator${
            item.positionSuffix
          }.`
        );
    });
...
export function mallocCost(output: ProgramOutput): ProgramOutput {
  return {
    value: output.value,
    cost: output.cost + BigInt(output.value.atom.length) * costs.mallocPerByte,
  };
}
...
export function defaultUnknownOperator(
  ...
  if (costFunction === 0) cost = 1n;
    ...
    for (const item of args.toList()) {
      if (!item.isAtom)
        throw new Error(`Expected atom argument${item.positionSuffix}.`);
      argSize += item.atom.length;
      cost += costs.arithPerArg;
    }
    cost += BigInt(argSize) * costs.arithPerByte;
    ...
    if (argList.length) {
      ...
      if (!first.isAtom)
        throw new Error(`Expected atom argument${first.positionSuffix}.`);
