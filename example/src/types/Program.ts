...
export class Program {
  ...
  public hash(): Uint8Array {
    return this.isAtom
      ...
      : hash256(
          concatBytes(Uint8Array.from([2]), this.first.hash(), this.rest.hash())
        );
  }
  ...
  public define(program: Program): Program {
    ...
    if (this.isAtom || this.first.isCons || this.first.toText() !== 'mod')
      result = Program.fromList([Program.fromText('mod'), Program.nil, this]);
    ...
    if (this.isAtom) {
      if (this.isNull) return '()';
        ...
        try {
          ...
          for (let i = 0; i < string.length; i++) {
            if (!printable.includes(string[i])) return `0x${this.toHex()}`;
          }
      ...
      if (showKeywords && this.first.isAtom) {
        ...
        const keyword = Object.keys(keywords).find(
          (keyword) => keywords[keyword as keyof typeof keywords] === value
        );
