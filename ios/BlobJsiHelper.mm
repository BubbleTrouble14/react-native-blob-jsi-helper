...
RCT_EXPORT_BLOCKING_SYNCHRONOUS_METHOD(install)
{
  ...
  // unsigned char message[] = "Hello, BLST!";
  // size_t message_len = strlen((char*)message);

  //   // Generate a random private key
  // blst_scalar sk;

    auto addNumbersTest = jsi::Function::createFromHostFunction(runtime,
                                                                     jsi::PropNameID::forUtf8(runtime, "addNumbersTest"),
                                                                     2,
                                                                     [](jsi::Runtime& runtime,
        ...
        int n1 = static_cast<int>(args[0].asNumber());
        int n2 = static_cast<int>(args[1].asNumber());

        int newVal = add_numbers(n1, n2);

        return jsi::Value(static_cast<double>(newVal));
  });
  runtime.global().setProperty(runtime, "addNumbersTest", addNumbersTest);
